...
<div id="tempSearchColums" style="display: none;">
  ...
  <t:datagrid name="sXyCheckApplyListCheck" checkbox="false" fitColumns="true" title="享宇考勤异常审批" actionUrl="sXyCheckApplyController.do?datagridCheck" 
   ...
   <t:dgFunOpt funname="goAgreeCheck(id)" title="批准" exp="isApprove#eq#00"/>
   <t:dgFunOpt funname="goNotAgreeCheck(id)" title="否决" exp="isApprove#eq#00"/>
 ...
 $(document).ready(function(){
		//给时间控件加上样式
			$("#sXyCheckApplyListChecktb").find("input[name='applyDate_begin']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
			$("#sXyCheckApplyListChecktb").find("input[name='applyDate_end']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
			$("#sXyCheckApplyListChecktb").find("div[name='searchColums']").find("form#sXyCheckApplyListCheckForm").append($("#tempSearchColums div[name='searchColums']").html());
		    $("#tempSearchColums").html('');
});
//批准
 function goAgreeCheck(id) {
	 var url = 'sXyCheckApplyController.do?goAgreeCheck&id='+id;
	 createwindow("选择下一审批人", url, 600, 300);
 }
 //否决
 function goNotAgreeCheck(id) {
	 var url = 'sXyCheckApplyController.do?goNotAgreeCheck&id='+id;
