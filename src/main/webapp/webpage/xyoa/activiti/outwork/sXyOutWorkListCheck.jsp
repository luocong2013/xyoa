...
<div id="tempSearchColums" style="display: none;">
  ...
  <t:datagrid name="sXyOutWorkListCheck" checkbox="false" fitColumns="true" title="享宇外出审批表" actionUrl="sXyOutWorkController.do?datagridCheck" 
   ...
   <t:dgFunOpt funname="goAgreeOutwork(id)" title="批准" exp="isApprove#eq#00"></t:dgFunOpt>
   <t:dgFunOpt funname="goNotAgreeOutwork(id)" title="否决" exp="isApprove#eq#00"></t:dgFunOpt>
 ...
 $(document).ready(function(){
		//给时间控件加上样式
			$("#sXyOutWorkListChecktb").find("input[name='outStartTime_begin']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
			$("#sXyOutWorkListChecktb").find("input[name='outStartTime_end']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
			$("#sXyOutWorkListChecktb").find("input[name='outEndTime_begin']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
			$("#sXyOutWorkListChecktb").find("input[name='outEndTime_end']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
			$("#sXyOutWorkListChecktb").find("div[name='searchColums']").find("form#sXyOutWorkListCheckForm").append($("#tempSearchColums div[name='searchColums']").html());
 	        $("#tempSearchColums").html('');
});
//批准申请
 function goAgreeOutwork(id) {
	 var url = 'sXyOutWorkController.do?goAgreeOutwork&id='+id;
	 createwindow("选择下一审批人", url, 600, 300);
 }
 
 //否决申请
 function goNotAgreeOutwork(id) {
	 var url = 'sXyOutWorkController.do?goNotAgreeOutwork&id='+id;
