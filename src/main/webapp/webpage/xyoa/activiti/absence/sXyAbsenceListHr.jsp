...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="sXyAbsenceListHr" checkbox="false" fitColumns="true" title="享宇请假HR审批表"  actionUrl="sXyAbsenceController.do?datagridHr" 
   ...
   <t:dgFunOpt funname="goHrAgreeAbsence(id)" title="批准" exp="isApprove#eq#00"/>
   <t:dgFunOpt funname="goHrNotAgreeAbsence(id)" title="否决" exp="isApprove#eq#00"/>
 ...
 $(document).ready(function(){
		//给时间控件加上样式
			$("#sXyAbsenceListHrtb").find("input[name='startTime_begin']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
			$("#sXyAbsenceListHrtb").find("input[name='startTime_end']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
			$("#sXyAbsenceListHrtb").find("input[name='endTime_begin']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
			$("#sXyAbsenceListHrtb").find("input[name='endTime_end']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
			$("#sXyAbsenceListHrtb").find("div[name='searchColums']").find("form#sXyAbsenceListHrForm").append($("#tempSearchColums div[name='searchColums']").html());
 	        $("#tempSearchColums").html('');
});
//HR批准
 function goHrAgreeAbsence(id) {
	 var url = 'sXyAbsenceController.do?goHrAgreeAbsence&id='+id;
	 createwindow("批准原因", url, 600, 250);
 }
 
 //HR否决
 function goHrNotAgreeAbsence(id) {
	 var url = 'sXyAbsenceController.do?goHrNotAgreeAbsence&id='+id;
