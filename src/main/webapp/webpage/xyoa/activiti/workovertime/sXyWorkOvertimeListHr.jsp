...
<div id="tempSearchColums" style="display: none;">
  ...
  <t:datagrid name="sXyWorkOvertimeListHr" checkbox="false" fitColumns="true" title="享宇加班表" actionUrl="sXyWorkOvertimeController.do?datagridHr" 
   ...
   <t:dgFunOpt funname="goHrAgreeOvertime(id)" title="批准" exp="isApprove#eq#00"/>
   <t:dgFunOpt funname="goHrNotAgreeOvertime(id)" title="否决" exp="isApprove#eq#00"/>
 ...
 $(document).ready(function(){
		//给时间控件加上样式
			$("#sXyWorkOvertimeListHrtb").find("input[name='applyStartTime_begin']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
			$("#sXyWorkOvertimeListHrtb").find("input[name='applyStartTime_end']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
			$("#sXyWorkOvertimeListHrtb").find("input[name='applyEndTime_begin']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
			$("#sXyWorkOvertimeListHrtb").find("input[name='applyEndTime_end']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
			$("#sXyWorkOvertimeListHrtb").find("div[name='searchColums']").find("form#sXyWorkOvertimeListHrForm").append($("#tempSearchColums div[name='searchColums']").html());
		    $("#tempSearchColums").html('');
});
 //HR批准加班申请
 function goHrAgreeOvertime(id) {
	 var url = 'sXyWorkOvertimeController.do?goHrAgreeOvertime&id='+id;
	 createwindow("批准原因", url, 600, 250);
 }
 
 //HR否决加班申请
 function goHrNotAgreeOvertime(id) {
	 var url = 'sXyWorkOvertimeController.do?goHrNotAgreeOvertime&id='+id;
