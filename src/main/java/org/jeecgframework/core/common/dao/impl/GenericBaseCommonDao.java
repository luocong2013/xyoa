...
//		}
	...
	public DataGridReturn getDataGridReturn(final CriteriaQuery cq,
		...
		Projection projection = impl.getProjection();
		int allCounts = 0;
		Object objectCounts = criteria.setProjection(
				Projections.rowCount()).uniqueResult();
		
		if(objectCounts != null){
			allCounts = ((Long)objectCounts).intValue();
		}
		criteria.setProjection(projection);
		if (projection == null) {
			criteria.setResultTransformer(CriteriaSpecification.ROOT_ENTITY);
	...
	public DataGrid getDataGridForHSql(final Class  className, final String hSql, final DataGrid dataGrid,final boolean isOffset){
		...
		if(isOffset){
			...
			dataGrid.setResults(query.list());
	 ...
	 */
	@Override
	public DataGrid getDataGridForHQL(final String hql, final DataGrid dataGrid, final int beginNum, final int pageSize, final Object... param) {
		Query query = getSession().createQuery(hql);
		if (param != null && param.length > 0) {
			for (int i = 0; i < param.length; i++) {
...
//		dataGrid.setTotal(allCounts);
	 ...
	 */
	@Override
	public int getTotalForHQL(String hql, Object... param) {
		Query query = getSession().createQuery(hql);
		if (param != null && param.length > 0) {
			for (int i = 0; i < param.length; i++) {
				query.setParameter(i, param[i]);
