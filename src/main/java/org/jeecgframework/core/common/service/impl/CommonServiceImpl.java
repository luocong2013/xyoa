...
//				+ "(d = (select uo.tsDepart from TSUserOrg uo where uo.tsUser = ?)) "
//				+ "or (d.TSPDepart = (select uo.tsDepart from TSUserOrg uo where uo.tsUser = ?)))";
	...
	public List<TSUser> findHqlTSUsersByUR(TSUser tsUser) {
		String hql = "select ru.TSUser from TSRoleUser ru where ru.TSUser <> ? and ru.TSRole.roleCode in ('employee', 'outEm', 'headman', 'hr', 'dm', 'hrdm', 'vice')";
		return this.findHql(hql, tsUser);
	}
	...
	public String startFlow(String processDefinitionKey, String Id, Map<String, Object> variables, String applyUserId) throws Exception {
		...
		ProcessInstance processInstance = runtimeService.startProcessInstanceByKey(processDefinitionKey, Id, variables);
	...
	public List<Task> queryTask(String assignee, boolean isHr) throws Exception {
		...
		if(!isHr) {
			tasks = taskService.createNativeTaskQuery()
					.sql("SELECT * FROM " + managementService.getTableName(Task.class) + " T WHERE T.NAME_ <> #{taskName} AND T.ASSIGNEE_ = #{assignee}")
					.parameter("taskName", "人事部")
		...
		if (!isHr) {
			historicTaskInstances = historyService.createNativeHistoricTaskInstanceQuery()
					.sql("SELECT * FROM " + managementService.getTableName(HistoricTaskInstance.class) +
							" H WHERE H.NAME_ <> #{taskName} AND H.ASSIGNEE_ = #{assignee} AND H.END_TIME_ IS NOT NULL")
					.parameter("taskName", "人事部")
					.parameter("assignee", assignee)
					.list();
		} else {
			historicTaskInstances = historyService.createNativeHistoricTaskInstanceQuery()
					.sql("SELECT * FROM " + managementService.getTableName(HistoricTaskInstanceEntity.class) +
							" H WHERE H.NAME_ = #{taskName} AND H.ASSIGNEE_ = #{assignee} AND H.END_TIME_ IS NOT NULL")
					.parameter("taskName", "人事部")
					.parameter("assignee", assignee)
					.list();
	...
	public List<Map<String, Object>> queryRunFlowInstId(String tableName, String assignee, boolean isHr, int curPage,
		...
		String sql = null;
		if (isHr) {
			sql = "SELECT art.PROC_INST_ID_ " +
					 "FROM act_ru_task art " +
					 "INNER JOIN "+tableName+" s ON art.PROC_INST_ID_ = s.flow_inst_id " +
					 "WHERE art.NAME_ = '人事部' AND art.ASSIGNEE_ = ? " +
					 "ORDER BY s.c_time DESC " +
					 "LIMIT ?, ?";
		} else {
			sql = "SELECT art.PROC_INST_ID_ " +
					 "FROM act_ru_task art " +
	...
	public int queryRunTotal(String tableName, String assignee, boolean isHr) throws Exception {
		String sql = null;
		if (isHr) {
			sql = "SELECT COUNT(art.PROC_INST_ID_) " +
					 "FROM act_ru_task art " +
					 "INNER JOIN "+tableName+" s ON art.PROC_INST_ID_ = s.flow_inst_id " +
					 "WHERE art.NAME_ = '人事部' AND art.ASSIGNEE_ = ?";
		} else {
	...
	public List<Map<String, Object>> queryHisFlowInstId(String tableName, String assignee, boolean isHr, int curPage,
		...
		String sql = null;
		if (isHr) {
			sql = "SELECT DISTINCT aht.PROC_INST_ID_ " +
					 "FROM act_hi_taskinst aht " +
					 "INNER JOIN "+tableName+" s ON aht.PROC_INST_ID_ = s.flow_inst_id " +
	...
	public int queryHisTotal(String tableName, String assignee, boolean isHr) throws Exception {
		String sql = null;
		if (isHr) {
			sql = "SELECT COUNT(DISTINCT aht.PROC_INST_ID_) " +
					 "FROM act_hi_taskinst aht " +
					 "INNER JOIN "+tableName+" s ON aht.PROC_INST_ID_ = s.flow_inst_id " +
					 "WHERE aht.NAME_ = '人事部' AND aht.ASSIGNEE_ = ? AND aht.END_TIME_ IS NOT NULL " +
					 "AND aht.DELETE_REASON_ <> 'deleted'";
	...
	public void completeTask(String flowInstId, Map<String, Object> variables, String assignee, String comment)
		...
		Task task = taskService.createTaskQuery()
				.processInstanceId(flowInstId)
				.taskAssignee(assignee)//保证并发流程可以得到唯一的Task
				.singleResult();
		if(comment != null) {
	...
	public List<CommentBean> findComment(String flowInstId) throws Exception {
		...
		List<HistoricTaskInstance> historicTaskInstances = historyService.createHistoricTaskInstanceQuery()
				.processInstanceId(flowInstId)
				.orderByHistoricTaskInstanceEndTime()//以结束时间排序
				.desc()//降序排序
				.finished()//查询已完成的任务节点
				.list();
		...
		if (comments != null && !comments.isEmpty()) {
			for (Comment comment : comments) {
				...
				Integer isPass = (Integer)this.findHistoryVarInst(comment.getProcessInstanceId(), comment.getTaskId());
				commentBean.setRealName(tsUser.getRealName());
				if (isPass == 6) {
					commentBean.setIsAgree("同意销假");
				} else if (isPass == 7) {
					commentBean.setIsAgree("否决销假");
				} else if (isPass == 0) {
					commentBean.setIsAgree("否决");
				} else {
					commentBean.setIsAgree("同意");
				}
	...
	public String getNextApplyNo(ApplyTypeEnum applyType,String tableName) throws Exception{
		...
		Map<String,Object> maxApplyNo = findOneForJdbc("select max(apply_no) as applyNo from "+tableName+" where apply_no like '%"+time.format(new Date())+"%'");
	...
	public <T> DataGrid getDataGridRunForHQL(final Class<T> className, final DataGrid dataGrid, final boolean isHr, 
		...
		String hql = null;
		if (isHr) {
			hql = "SELECT S FROM "+className.getSimpleName()+" S, TaskInstanceEntity H where S.flowInstId = H.procInstId  "
	...
	public <T> DataGrid getDataGridHisForHQL(final Class<T> className, final DataGrid dataGrid, final boolean isHr, 
		...
		String hql = null;
		if (isHr) {
			hql = "SELECT DISTINCT S FROM "+className.getSimpleName()+" S, HisTaskInstanceEntity H where S.flowInstId = H.procInstId  "
	...
	public <T> int getRunTotal(final Class<T> className, final boolean isHr, final String paramHql, final String orgId, final Object... param) {
		String hql = null;
		if (isHr) {
			hql = "SELECT COUNT(S) FROM "+className.getSimpleName()+" S, TaskInstanceEntity H where S.flowInstId = H.procInstId  "
					+ "AND H.name = '人事部'  AND H.assignee = ?";
		} else {
			hql = "SELECT COUNT(S) FROM "+className.getSimpleName()+" S, TaskInstanceEntity H where S.flowInstId = H.procInstId  "
					+ "AND H.name <> '人事部'  AND H.assignee = ? AND H.assignee <> S.applySttaffId";
	...
	public <T> int getHisTotal(final Class<T> className, final boolean isHr, final String paramHql, final String orgId, final Object... param) {
		String hql = null;
		if (isHr) {
			hql = "SELECT COUNT(DISTINCT S) FROM "+className.getSimpleName()+" S, HisTaskInstanceEntity H where S.flowInstId = H.procInstId  "
					+ "AND H.name = '人事部'  AND H.assignee = ? AND H.endTime IS NOT NULL AND H.deleteReason <> 'deleted'";
		} else {
			hql = "SELECT COUNT(DISTINCT S) FROM "+className.getSimpleName()+" S, HisTaskInstanceEntity H where S.flowInstId = H.procInstId  "
					+ "AND H.name <> '人事部'  AND H.assignee = ? AND H.assignee <> S.applySttaffId AND H.endTime IS NOT NULL AND H.deleteReason <> 'deleted'";
		...
		for(int i=0;i<listDepart.size();i++)
		{
			if(listDepart.get(i).getDepartname()==null||"".equals(listDepart.get(i).getDepartname()))
				...
				if(listDepart.get(i).getOrgCode().length()>9){
					replacedepart+=listDepart.get(i).getTSPDepart().getDepartname()+"/"+listDepart.get(i).getDepartname()+"_"+listDepart.get(i).getId()+",";
				}else{
