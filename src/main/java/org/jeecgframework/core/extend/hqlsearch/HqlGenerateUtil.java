...
public class HqlGenerateUtil {
	 ...
	 *            request参数集合，封装了所有查询信息
	 */
	public static String installHqlForFlow(String tabName,CriteriaQuery cq, Object searchObj,
			Map<String, String[]> parameterMap) {
		return installHqlJoinAliasForFlow(tabName,cq, searchObj, getRuleMap(), parameterMap, "");
	}
	
	private static String installHqlJoinAliasForFlow(String tabName,CriteriaQuery cq, Object searchObj,
			...
			try {
				...
				if (type.contains("class java.lang")
					...
					if (value != null && !value.equals("")) {
						...
						if (value == null || rule == null || "class".equals(name)) {
							continue;
						}
						
						sql += tabName +"."+aliasName+ rule.getValue() + "'"+value +"'" +" AND " ;
					} else if (parameterMap != null) {
						if ("class java.lang.Integer".equals(type) || 
								"class java.math.BigDecimal".equals(type)||
								"class java.lang.Short".equals(type)||
								"class java.lang.Long".equals(type)||
								"class java.lang.Float".equals(type)) {
							if (beginValue != null) 
								sql += tabName +"."+aliasName+ HqlRuleEnum.GE.getValue() +beginValue +" AND " ;
							
							if (endValue != null) 
								sql += tabName +"."+aliasName+ HqlRuleEnum.LE.getValue() +endValue  +" AND " ;
					...
					if (StringUtils.isNotBlank(beginValue)) {
						sql += tabName +"."+aliasName+ HqlRuleEnum.GE.getValue() +"'"+beginValue+" 00:00:00'"  +" AND " ;
					}
					if (StringUtils.isNotBlank(endValue)) {
						sql += tabName +"."+aliasName+ HqlRuleEnum.LE.getValue() +"'"+endValue+" 23:23:59'"  +" AND " ;
					}
					if (isNotEmpty(value)) {
						sql += tabName +"."+aliasName+ HqlRuleEnum.EQ.getValue() +"'"+endValue+"'"  +" AND " ;
					}
					...
					if (isHaveRuleData(ruleMap, aliasName) ||( isNotEmpty(param)
						...
						if(StringUtils.isNotEmpty(installHqlJoinAliasForFlow(tabName,cq, param, ruleMap, parameterMap,
								aliasName)))
							sql +=  installHqlJoinAliasForFlow(tabName,cq, param, ruleMap, parameterMap,
									aliasName)  +" AND " ;
					}
				}
			} catch (Exception e) {
