...
public class SystemController extends BaseController {
	...
	@RequestMapping(params = "pcheck")
	public ModelAndView pcheck(HttpServletRequest request,String staffid) {
		request.setAttribute("staffid", staffid);
		return new ModelAndView("xyoa/pcheck/pcheckList");
	}
...
/**
	 * 文件列表
	 */
	@RequestMapping(params = "documentTabList")
	@ResponseBody
	public Map<String, Object> documentTabList(TSDocument doc, HttpServletRequest request, HttpServletResponse response,String staffid,DataGrid dataGrid,String isNotDel) {
		CriteriaQuery cq = new CriteriaQuery(TSDocument.class, dataGrid);
		if(!"".equals(doc.getDocumentTitle())&&doc.getDocumentTitle()!=null)
			doc.setDocumentTitle("*"+doc.getDocumentTitle()+"*");
		String typecode = oConvertUtils.getString(request.getParameter("typecode"));
	...
	//	TagUtil.datagrid(response, dataGrid);
		...
		List <TSDocument> result=dataGrid.getResults();
		List <TSDocument> list=new ArrayList<>();
		
		for(TSDocument entity:result)
		{
			TSDocument doct=new TSDocument();
			doct.setId(entity.getId());
			doct.setDocumentTitle(entity.getDocumentTitle());			
			doct.setDocumenttype(entity.getDocumenttype());
			SimpleDateFormat sf=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			if(entity.getCreatedate()!=null)		
			doct.setBusinessKey(sf.format(entity.getCreatedate()));//创建时间 
			doct.setSubclassname(entity.getSubclassname());
			//删除操作
			if(!"1".equals(isNotDel))
				doct.setNote("<button   onclick=\"delObj('"+entity.getId()+"');\" >删除</button>"
					+ "|<button  onclick=\"downObj('"+entity.getId()+"','"+entity.getSubclassname()+"');\" >下载</button> "  );
			else
				doct.setNote("<button  onclick=\"downObj('"+entity.getId()+"','"+entity.getSubclassname()+"');\" >下载</button> "  );
			//编辑操作
			doct.setRealpath("<button  class=\"easyui-linkbutton\" onclick=\"downObj('"+entity.getId()+"','"+entity.getSubclassname()+"');\" >下载</button> " );
